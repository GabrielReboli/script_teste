...
\connect postgresql://gabrielreboli:202cb962ac59075b964b07152d234b70@localhost/uvv

-- Criar o esquema lojas e autorizar o usuário a utilizar o esquema
CREATE SCHEMA lojas 
  AUTHORIZATION gabrielreboli;
...
ALTER ROLE gabrielreboli SET SEARCH_PATH TO lojas, public;
...
COMMENT ON TABLE  lojas                           IS 'Tabela que armazena as lojas a dispor.';

-- Incluir comentários nos metadados para descrever as colunas da tabela lojas
COMMENT ON COLUMN lojas.loja_id                   IS 'Chava primária da tebela lojas. Identifica a loja.';
COMMENT ON COLUMN lojas.nome                      IS 'Nome da loja.';
COMMENT ON COLUMN lojas.endereco_web              IS 'Endereço eletrônico do site referente à loja.';
COMMENT ON COLUMN lojas.endereco_fisico           IS 'Endereço completo da loja física.';
COMMENT ON COLUMN lojas.latitude                  IS 'Latitude da loja. Distancia em graus da loja e a linha do equador.';
COMMENT ON COLUMN lojas.longitude                 IS 'Longitude da loja. Distância em graus medida entre a loja e o Meridiano de Greenwich.';
COMMENT ON COLUMN lojas.logo                      IS 'Logotipo que identifica e representa a loja.';
COMMENT ON COLUMN lojas.logo_mime_type            IS 'Formato de imagem da logo.';
COMMENT ON COLUMN lojas.logo_arquivo              IS 'Armazena o nome do arquivo da logo.';
COMMENT ON COLUMN lojas.logo_charset              IS 'Conjunto de caracteres da logotipo.';
COMMENT ON COLUMN lojas.logo_ultima_atualizacao   IS 'Data da última atualização da logotipo.';
...
COMMENT ON TABLE estoques                 IS 'Armazena as informações referentes aos estoques de produtos da lojas.';

-- Incluir comentários nos metadados para descrever as colunas da tabela estoques
COMMENT ON COLUMN estoques.estoque_id     IS 'Chave primária da tabela estoques. Identifica o estoque.';
COMMENT ON COLUMN estoques.loja_id        IS 'Chave estrangeira da tebla lojas. Identifica a loja.';
COMMENT ON COLUMN estoques.produto_id     IS 'Chave estrangeira da tabela produtos. Identifica o produto.';
...
COMMENT ON TABLE  clientes                IS 'Tabela de clientes. Contém o id do cliente, email, nome e telefones para contato.';

-- Incluir comentários nos metadados para descrever as colunas da tabela clientes
COMMENT ON COLUMN clientes.cliente_id     IS 'Chave primária da tabela clientes. Identifica cada cleinte.';
COMMENT ON COLUMN clientes.email          IS 'Endereço de email do cliente.';
COMMENT ON COLUMN clientes.nome           IS 'Nome completo do cliente como no RG.';
COMMENT ON COLUMN clientes.telefone1      IS 'Telefone primário para contato com o cliente.';
COMMENT ON COLUMN clientes.telefone2      IS 'Telefone secundário para contato com o cliente.';
COMMENT ON COLUMN clientes.telefone3      IS 'Telefone terciário para contato com o cliente.';
...
COMMENT ON TABLE  envios                      IS 'Armazena os envios realizados.';

-- Incluir comentários nos metadados para descrever as colunas da tabela envios
COMMENT ON COLUMN envios.envio_id             IS 'Chave primária da tebela envios. Identifica o envio.';
COMMENT ON COLUMN envios.loja_id              IS 'Chave estrangeira da tabela lojas. Identifica a loja.';
COMMENT ON COLUMN envios.cliente_id           IS 'Chave estrangeira da tabela clientes. Identifica o cliente.';
COMMENT ON COLUMN envios.endereco_entrega     IS 'Endereço completo do destino que deverá ser realizada a entrega do pedido.';
COMMENT ON COLUMN envios.status               IS 'Estado do envio. Identifica as etapas do processo.';
...
COMMENT ON COLUMN pedidos.pedido_id   IS 'Chave primária da tabela. Identifica o pedido.';
COMMENT ON COLUMN pedidos.data_hora   IS 'Data e horário da realização do pedido.';
COMMENT ON COLUMN pedidos.cliente_id  IS 'Chave estrangeira da tabela clientes. Identifica o cliente.';
COMMENT ON COLUMN pedidos.status      IS 'Estado do pedido.';
COMMENT ON COLUMN pedidos.loja_id     IS 'Chave estrangeira da tabela lojas. Identifica a loja.';
...
COMMENT ON TABLE  pedidos_itens                   IS 'Armazena os itens de cada pedido.';

-- Incluir comentários nos metadados para descrever as colunas da tabela pedidos_itens
COMMENT ON COLUMN pedidos_itens.pedido_id         IS 'Chave primária da tabela pedidos_itens. Chave estrangeira da tabela pedidos. Identifica o pedido.';
COMMENT ON COLUMN pedidos_itens.produto_id        IS 'Chave primária da tabela pedidos_itens. Chave estrangeira da tabela produtos. Identifica o produto.';
COMMENT ON COLUMN pedidos_itens.numero_da_linha   IS 'Número da linha onde se encontra o item no determinado pedido.';
COMMENT ON COLUMN pedidos_itens.preco_unitario    IS 'Preço unitário do produto.';
COMMENT ON COLUMN pedidos_itens.quantidade        IS 'Quantidade de determinado item no pedido.';
COMMENT ON COLUMN pedidos_itens.envio_id          IS 'Chave estrangeira da tabela envios. Identifica o envio.';
